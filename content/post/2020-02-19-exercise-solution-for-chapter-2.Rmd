...
codon_compared %>% 
  ...
  mutate(rank = 1:n())
```
As you may notice, these p-values are large, negative (so very tiny) values. In calculating the p-value from chi-squared text statistic, we took the log. If you exponentiate the p-value you will get the actual p-value, but this number is so small (0.00000000e+00) that it is not meaningful. Instead, we will leave the p-value as a negative value and sort the amino acids from most negative to less negative. We now have the amino acids ranked from most biased codons (1) to least (19). 
