...
PlantGrowth %>% 
  ...
  tidy()
```
Permutation test data resample: 
```{r}
pg1 <- PlantGrowth %>% 
  sample_frac(size = 0.5) 
...
```{r}
library(kableExtra)
data <- c('original', 'doubled w/o random noise', 'same size as original, half observations replicated', 'half, random noise added to each replicate')
n <- c(30, 60, 30 ,30)
