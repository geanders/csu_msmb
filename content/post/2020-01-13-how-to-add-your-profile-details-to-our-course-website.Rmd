...
are doing. These commits are only saved in your local branch. To send them up to 
the remote branch of the repository on GitHub, you'll need to push those commits. 
Once you push your local commits, your GitHub repository should exactly mirror
...
like to include in the commit. Then write a short **commit message**
...
`blogdown` package to render the website. This will *only* update and
...
will open the website in your default web browser. Take a look at the web
address when you do---it should start with `127.0.0.1`. This is a **loopback
address**---an IP address that refers back to your local computer
(**localhost**), rather than an outside web servers.
Anytime you're building a website and checking it locally, you'll see this as
the web address when you open the site in a web browser. (Should you want a
humourously geeky T-shirt, you can [get one with "There's no place like
...
and use *the same name* ("avatar.jpg").

It would be helpful for you to use a photo of yourself, since that will help 
us get to know each others' names, but if you don't have one or would prefer
...
[RStudio's help documention on the
